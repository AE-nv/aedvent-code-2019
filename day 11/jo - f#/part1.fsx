...
let rec run (runner : IntCode.State -> IntCode.State) (state : State) =
    ...
    | IntCode.ProgramMode.Finished -> state
    | _ ->
        let progstate = runner state.programState
        let [direction;new_color] = progstate.output
        ...
        match progstate.mode with
        | IntCode.Finished -> next
        | IntCode.AwaitingInput -> run runner next
...
let aProgramState outputs = {IntCode.initState [] [] with mode = IntCode.Finished; output = outputs |> List.rev }
